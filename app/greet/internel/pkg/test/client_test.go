...
var (
	...
	fileName  = "greetService/config.json" //文件名称
)
...
func TestClientHello(t *testing.T) {
	...
	newClient := service.NewGreetClient(config, service.NewDiscover(config, klog.DefaultLogger()), klog.DefaultLogger())
	...
	resp, err := newClient.Greet(ctx, &v1.Request{Message: "Hi,polaris!"})
